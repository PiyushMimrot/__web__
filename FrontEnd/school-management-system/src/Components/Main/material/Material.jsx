...
const Material = () => {
  const topicId = useParams().materialId;
  ...
  const fetchInfo = async () => {
    try {
      const materialResponse = await fetch(
        `${SERVER}/knowledgebank/getbank/${topicId}`,
        {
          credentials: "include",
        }
      );
      ...
      const topicResponse = await fetch(`${SERVER}/topic/getone/${topicId}`, {
        credentials: "include",
      });
      if (!topicResponse.ok) {
      ...
      const chapterResponse = await fetch(
        `${SERVER}/chapter/getone/${topicData.chapter_id}`,
        {
          credentials: "include",
        }
      );
      ...
      const topicDataResponse = await fetch(`${SERVER}/topic/getall`, {
        credentials: "include",
      });
      if (!topicDataResponse.ok) {
        throw new Error(`HTTP error! Status: ${topicDataResponse.status}`);
      }
      const alltopics = await topicDataResponse.json();

      setChapter(chapterData);
      setKnowledgeBank(materialData);
      setTopic(topicData);
      setTopicsTag(alltopics);
    } catch (error) {
                      ...
                      <div className="card teacher-card">
                        <div className="card-body d-flex">
                          ...
                          <div className="teacher-info border-start ps-xl-4 ps-md-3 ps-sm-4 ps-4 w-100">
                            ...
                            <div className="video-setting-icon mt-3 pt-3 border-top d">
                              <p>{ele.desc}</p>
                            </div>
                          </div>
                        </div>

                        <div style={{paddingInline: "9px"}} className="d-flex flex-column align-items-start justify-content-center">
                          ...
                          <div className="d-flex flex-wrap gap-2">
                            {ele.tags?.map((tag, ind) => (
                              <span
                                key={ind}
                                className="light-warning-bg py-1 px-2 rounded-1 d-inline-block fw-bold mb-1" // Adjusted margin for better spacing
                                style={{ fontSize: "10px" }}
                              >
                                {tag}
                              </span>
